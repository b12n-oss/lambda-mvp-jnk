(ns script.bench-run
  "Orchestrates `bb bench`: for each memory tier, force a fresh
  execution environment (any config update does this), measure one
  cold sample, then BENCH_WARM_SAMPLES back-to-back warm samples, and
  print a comparison table. See docs/guide/cold-warm-boot.md -- and
  note its caveat that these numbers are NOT directly comparable to
  the Jolt sibling's own numbers, since a container-image function's
  cold-start profile differs from a zip-based custom runtime's.")
...
(defn- invoke-sample! []
  (let [out-file (str (System/getProperty "java.io.tmpdir")
                      "/lambda-mvp-jnk-bench-" (System/nanoTime) ".json")
        {:keys [exit out err]}
  ...
  (let [cold (invoke-sample!)]
    ...
    (let [warm (mapv (fn [_] (invoke-sample!)) (range warm-samples))]
      ...
      (when (some :init-duration-ms warm)
        (println "lambda-mvp-jnk: WARNING -- a 'warm' sample for" tier
